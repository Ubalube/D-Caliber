...
public class GunHybrid extends GunBase implements IHasModel
{
	 ...
	 * @param strength
	 */
	public GunHybrid(String name, CreativeTabs tab, int fireRate, int ammocap, int reloadtm, int recoil, float bulletDamage, int bulletDuration, Item ammunition, int guntype, String desc, String ammoN, int strength) 
	{
		...
		this.addPropertyOverride(new ResourceLocation("hybrid"), new IItemPropertyGetter()
        {
	        ...
	        public float apply(ItemStack stack, @Nullable World worldIn, @Nullable EntityLivingBase entityIn)
	        {
	            ...
	            float j = nbt.getBoolean("hybrid") ? 1.0F : 0.0F;
	            return j; 
	        }
        });
	...
	public void checkStates(ItemStack stack, World worldIn, Entity entityIn) {
        ...
        if (!nbt.hasKey("hybrid")) {
            nbt.setBoolean("hybrid", false);
        }
		super.checkStates(stack, worldIn, entityIn);
	...
	public void toggleHybrid(boolean isOn, ItemStack stack, NBTTagCompound nbt)
	{
		nbt.setBoolean("hybrid", isOn);
	}
	
	public boolean getHybrid(ItemStack stack, NBTTagCompound nbt)
	{
		return nbt.getBoolean("hybrid");
	}
